...
links <- readr::read_csv("links.csv") %>%
  filter(!(is.na(DailyCount)), HCMType != "CC")
...
links[, "Volume_Group"] <- links[, volume_var]
...
```{r mdd}
plotly_mdd(links, volume_var, count_var, color_field = "facility_group")
```
...
link_stats_table(links, volume_var, count_var, group_field = "Volume_Group",
                 type = "rmse") %>%
